...
firstRepeat xs = eval (f' xs) S.empty where

  f' :: Ord a => List a -> State (S.Set a) (Optional a)
  f' (y:.ys) = do
    set <- get
    case S.member y set of
      False -> do
         put (S.insert y set)
         f' ys
      True -> pure $ Full y 
...
listWithState :: Ord a1 => 
  ...
  t -> 
  a 
listWithState f p xs = eval (f (State . lift2 (lift2 (,)) p S.insert) xs) S.empty
...
listWithState' :: Ord a1 => 
  ...
  t -> 
  a 
listWithState' f p = flip eval S.empty . (f (State . lift2 (A.&&&) p S.insert))
...
-- True
isHappy :: Int -> Bool 
isHappy num = eval (f' num) S.empty where
  f' n = do
    if n == 1 
    then pure True 
    else do
      set <- get
      case S.member n set of
        True -> pure False
        False -> do
          put (S.insert n set)
          f' . P.sum . P.map ((\x -> x * x) . digitToInt) . P.show $ n

-- suggested solution hides all details of state processing
isHappy' :: Int -> Bool 
isHappy' = contains 1 
  . firstRepeat 
  . produce (P.sum . P.map ((\x -> x * x) . digitToInt) . P.show)
