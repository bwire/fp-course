...
tok p = p >>= (\v -> spaces >>= (\_ -> valueParser v)) 

-- or suggested approach (using do-notation)
tok' :: Parser a -> Parser a
tok' p = do
  v <- p
  spaces
...
-- True
hex :: Parser Char
hex = chr . hInt <$> replicateA 4 (satisfy isHexDigit) where
  hInt s = 
    case readHex s of
      Empty -> 0
      Full n -> n
...
sepby pv pd = sepby1 pv pd ||| pure Nil
...
satisfyAll px = satisfy (and . sequence px)
...
satisfyAny px = satisfy (or . sequence px)
